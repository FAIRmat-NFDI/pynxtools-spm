...
=== "A Bruker (SPMLab) AFM Raw Data File"
    <p>A FLT file is a binary file that starts with an INI style text header. Only that header is shown below; the image data follows it directly after the byte given by <b>DataOffset</b>.</p>
    <div class="scrollable">
    ```ini
    --8<-- "included_file_content/bruker_afm/flt_header.txt"
    ```
    </div>
