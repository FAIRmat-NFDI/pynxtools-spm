...
### __Raw Data File__, file that is generated by the software of the instrument. These files may contain some metadata but not all the metadata that is required by the NeXus application definition.

### __ELN Schema File__,  yaml schema file that contains schema structure following the NeXus application defintiion and metadata for NOMAD (for NOMAD internal use). NOMAD reads such schema file and render a GUI where user can fill the metadata corresponding the NeXus schema. Later, nomad generates structural a yaml file which will directly read by the raader.

Typical ELN schema file and it's Graphical Representation in NOMAD:

!!!note

    An schema file can be added to NOMAD upload which will successfully generate an archive file (e.g. for shcema file `sts.scheme.archive.yaml` nomad will create `<UploadName>.archive.json` file). The data section of the `archive.json` file renders the complete schema structure defined in `schema.archive.yaml` file where user add the metadata of the experiments.

!!! example

    === "STS Schema File for NOMAD (sts.scheme.archive.yaml)"
