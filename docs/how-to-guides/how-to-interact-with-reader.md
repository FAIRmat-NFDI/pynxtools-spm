...
## __Input Files__
Reader has to be given some specific input files which are required for its operation. They shall be coming from the experiment instrument and ELN file with user provided meatadata or ELN schema file to write your metadata via NOMAD. Here we will discuss, the input files and how they are being used in different situations.
...
<!-- TODO: Add how raw path formated by parsers. -->
<!-- TODO: At the home page mentioned to use this package use must have good knowledge of NeXus symantics and applicaiton defintions of SPM -->


### __ELN Schema File__
A yaml schema file (with extension `.scheme.archive.yaml`) that contains schema structure according to the NeXus application defintiion and a few operational metadata for NOMAD (used by NOMAD). NOMAD reads such schema file and creates an archive file (with extention `archive.json`). Later, NOMAD renders a GUI representing the archive file where user can fill the metadata corresponding the NeXus schema. Later, nomad generates structural a yaml file (another type of eln file with extenstion `.yaml`) collecting the user provided data. The final eln file (with extension `.yaml`) will be read by the reader and data goes to the NeXus file (for visualization of this file process fllow `Drag and Drop Example in NOMAD` in [Use Reader in NOMAD](../tutorials/reader-use-in-nomad.md) ). 

In the tabbed window below, an example of ELN schema file for STS experiment is shown. The schema is designed according to the NeXus application definition `NXsts`. The schema file contains all required metadata fields that are not supplied by raw file but required by the `NXsts` application definition. Tab from left to right shows the schema file, the generated archive file and the graphical representation of the archive file in NOMAD and final eln file (with extension `.yaml`).
...
### __ELN YAML File__
 A eln yaml file is similar to the yaml (with extension `.yaml`) generated by NOMAD from the ELN schema file (with extension `.scheme.archive.yaml`). This ELN yaml file also contains the same structre of the application definition but in much more simpler than the __ELN Schema File__. Such file can be writen by user follwing the application defintion `NXsts` without help of NOMAD. This eln file only compatible to use with the reader invoked in command line interface(follow [Transfer raw data](../reference/transfer-raw-data.md) guide).
...
### __Config File__
Config file that carries mapping information from the raw data to the NeXus Application Definition concepts. The config file follows certain rules how the raw data will be processed and orgaised in the NeXus file. It is expected that users from lab should scrutinize the config file and modify it according to their need and the file is not needed to be modified by the underlying data structure but raw data paths.
...
__1.__ The simplest way to map the NeXus concepts (field and/or attribute) to the raw data file is a map from concept name to another map object.
...
__3.__ Use config file to define the variadic name for a concept like group, field, and attribute. In NeXus concepts, a fraction or the entire concept name can be modified while instantiating objects from concepts and therefore, multiple instances of the same concept can be created.
...
__4.__ Write customised NXdata group via config file. It is very ofen situation that a user needs to annotate plot by defining the axis name and title of plot.
...
__5.__ The `nxformatter` of `pynxtools-xpm` has common functions, methods and other programatic tools that use the common rules in config file as described above. The curated concpets shall be written in the `template` object of `pynxtools`. But, there may have some special cases where the generalised functions and methods shall not be working. In that case, we have some customised methods that take care of the special fields. To annotate which concepts or fields need special treatment, the config file has a key `#note`. The value of `#note` holds human readable description e.g., the name of the method that handles the special case. In class variable `_grp_to_func` must have a map form group name to the method name.
...
There are some default freatures to handle the raw data from specific the vendor files.
