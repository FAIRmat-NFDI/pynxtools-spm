...
An ELN YAML file is similar to the YAML file (with the extension `.yaml`) generated by NOMAD from the ELN schema file (with the extension `.scheme.archive.yaml`). This ELN YAML file also contains the same structure as the application definition but is much simpler than the __ELN Schema File__. Such a file can be written by the user following the application definition `NXsts` without the help of NOMAD. This ELN file is only compatible for use with the reader invoked via the command-line interface (see [Transfer Raw Data](../reference/standalone-usages.md) guide).
