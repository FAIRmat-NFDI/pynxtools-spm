...
=== "A Bruker (SPMLab) AFM Raw Data File"
    <p>A FLT file written by Bruker <b>SPMLab</b> (version <b>1.00</b>) is a binary file that starts with an INI style text header. The header is shown below; the image data follows it directly after the byte given by <b>DataOffset</b>. Each FLT file holds one channel (here <b>Height</b>, given by the header key <b>DataName</b>) of one scan direction (here <b>FORWARD</b>).</p>
    <div class="scrollable">
    ```ini
...
__2.__ In AFM, for Bruker `SPMLab` FLT raw files, the raw data keys are prefixed by the channel name of the file (e.g. `/Height/meta/SetPoint` for a file with `DataName=Height`). Since a config file cannot know that name in advance, the placeholder `/CHANNEL/` is used in the `raw_path` entries and is replaced by the channel prefix of the file (e.g. `/Height/`) when the config file is loaded.

=== "The `/CHANNEL/` placeholder in a raw path"
    <p>For a file with the header entry <b>DataName=Height</b>, the raw path <b>/CHANNEL/meta/X Transfer Coefficient</b> below is resolved to <b>/Height/meta/X Transfer Coefficient</b>. Use the placeholder in your own config file so that the same file works for every channel (e.g. <b>Height</b>, <b>Adhesion</b>).</p>
    <div class="scrollable">
    ```json
...
=== "Parse a Bruker file from AFM Experiment"
    <div class="scrollable">
    ```python
...
=== "Curate a Bruker file from AFM experiment"
    <div class="scrollable">
    ```python
