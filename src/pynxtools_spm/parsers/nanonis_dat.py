...
class DatGenericNanonis:
    ...
    def check_and_write_unit(
        self, dct, key_or_line, unit_separators, end_of_separators, value=None
    ):
        ...
        end_of_separators : list
            List of end separator chars
        value : dict, optional
            dict to store dict
        """
        for sep_unit, end_sep in zip(unit_separators, end_of_separators):
    ...
    def retrieve_key_recursively(
        self, line_to_analyse: str, dict_to_store: NestedDict_t, key_separators: list
    ) -> None:
        ...
        for k_sep in key_separators:
            ...
            if k_sep in line_to_analyse:
                ...
                self.check_and_write_unit(
                    dict_to_store, key, unit_separators, end_of_separators, new_dict
                )
                self.retrieve_key_recursively(rest, new_dict, key_separators)
                return

        for sep_unit in unit_separators:
            if sep_unit in line_to_analyse:
                self.check_and_write_unit(
                    dict_to_store, line_to_analyse, unit_separators, end_of_separators
                )
                return
    ...
    def check_matrix_data_block_has_started(
        ...
        Returns
        -------
            Bool flag: Flag for matrix data found
    ...
    def extract_and_store_from_dat_file(self) -> None:
        ...
        def dismantle_matrix_into_dict_key_value_list(
        ...
        with open(self.raw_file, mode="r", encoding="utf-8") as file_obj:
            ...
            for ind, line in enumerate(lines):
                ...
                if is_mat_data:
                    ...
                    is_matrix_data_found = False
                    dismantle_matrix_into_dict_key_value_list(
                        last_line, one_d_numpy_array, self.bias_spect_dict
                    )
                    last_line = line
                else:
                    self.retrieve_key_recursively(
                        last_line, self.bias_spect_dict, key_separators
                    )
                    last_line = line
